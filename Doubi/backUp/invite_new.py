...
activityId = '1520020257503784961'    # 活动id
authorCode = '1523584605943877634' # 助力码
shopid = '1000396688'   # 店铺id
activityUrl = f'https://lorealjdcampaign-rc.isvjcloud.com/interact/index?activityType=10006&templateId={templateId}&activityId={activityId}&nodeId=101001005&&shareUserId={authorCode}&adsource=null&shareuserid4minipg=null&shopid={shopid}'
...
async def getMember(ua,token):
    url = 'https://lorealjdcampaign-rc.isvjcloud.com/apps/interact/api/task/member/getMember'
    headers = {
...
async def login(ua,token):
    try:
        url = 'https://lorealjdcampaign-rc.isvjcloud.com/apps/interact/api/user-info/login'
        headers = {
        'Accept': 'application/json, text/plain, */*',
        'Accept-Encoding': 'gzip, deflate, br',
        'Accept-Language': 'zh-CN,zh-Hans;q=0.9',
...
async def bindWithVender(ua,cookie,body):
    try:
        url = f'https://api.m.jd.com/client.action?appid=jd_shop_member&functionId=bindWithVender&body={json.dumps(body)}&clientVersion=9.2.0&client=H5&uuid=88888&h5st=20220509205002795;0212287632151045;8adfb;tk02wcf3d1cbb18ncSu34ztnYv5aioR9vFMvYoxjiZdMrUaXTMWZG79kr807PFF6Oh3ghNNjhgj6clel65NjioE7XelS;8264e035683f0da5c99302c3071ff3ccc1594da4da2f2c2301e0b751ebe2e31e;3.0;1652100602795&jsonp=jsonp_1652100598901_54516'
        header = {
            'Host': 'api.m.jd.com',
            'Cookie': cookie,
            'Accept-Encoding': 'gzip, deflate, br',
...
async def checkopencard(token,ua,):
    url = 'https://lorealjdcampaign-rc.isvjcloud.com/apps/interact/api/join/check'
    headers = {
        "Host":"lorealjdcampaign-rc.isvjcloud.com",
        'Accept': 'application/json, text/plain, */*',
        'Accept-Encoding': 'gzip, deflate, br',
...
async def myself(token,ua,):
    url = 'https://lorealjdcampaign-rc.isvjcloud.com/apps/interact/api/task/bargain/guest/myself'
    headers = {
        "Host":"lorealjdcampaign-rc.isvjcloud.com",
        'Accept': 'application/json, text/plain, */*',
        'Accept-Encoding': 'gzip, deflate, br',
    ...
    for n,ck in enumerate(cks,1):
        ...
        if result['code'] == 200:
            ...
            if token:
                ...
                if cklogin:
                    if n == 1:
                        shopName = cklogin['data']['shopName'] # 店铺名字
                        actName = cklogin['data']['actName'] # 活动名字
                        nickName = cklogin['data']['nickName'] # 用户名
                        token = cklogin['data']['token'] # 活动ck
                        await asyncio.sleep(1)
                        result = await getMember(ua,token) # 活动详情信息
                        await asyncio.sleep(1)
                        prize = await prizeList(token,ua)
                        prizelists = []
                        for i in prize['data']['prizeInfo']:
                            bean = i['prizeName']
                            leftNum = i['leftNum']
                            position = i['position']
                            days = i['days']
                            if leftNum != 0:
                                needinviteNum.append(days)
                            msg = f'奖品{position}:  助力: {days}人，奖品: {bean}，库存: {leftNum}件\n'
                            prizelists.append(msg)
                        print(f'您好！[{nickName}]，当前活动[{actName}]信息!\n店铺：{shopName}\n默认全部助力[{authorCode}],需要助力{needinviteNum[-1]}次\n') 
                        if len(prizelists)!=0:
                            print('\n'.join(prizelists)+'\n')
                    else:
                        if success >= needinviteNum[-1]:
                            print('助力够了，去手动领奖吧')
                            return
                        await asyncio.sleep(1)
                        ...
                        if result['result']['userInfo']['openCardStatus']==0: # 0 未开卡
                            ...
                            if token:
                                ...
                                if cklogin:
                                    ...
                                    await asyncio.sleep(1)
                                    result = await bindWithVender(ua,ck,{"venderId":str(shopid),"shopId":str(shopid),"bindByVerifyCodeFlag":1,"registerExtend":{},"writeChildFlag":0,"channel":9006})
                                    print(result)
